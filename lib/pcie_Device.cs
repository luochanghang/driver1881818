...
#region " properties " 
        ...
        public WDC_ADDR_DESC[] AddrDesc
        {
            get
            {
                return m_wdcDevice.pAddrDesc;//WDC_DEVICE类成员返回 WDC_ADDR_DESC类型结构 的数组   WDC_ADDR_DESC描述bar空间
            }
            set
            {
...
#region " Interrupts "
        ...
        public DWORD DisableInterrupts()
        {
            ...
            dwStatus = DisableCardInts();//关闭卡中断 虚拟子方法 子类重写

            dwStatus = wdc_lib_decl.WDC_IntDisable(m_wdcDevice);//返回状态
            if (dwStatus != (DWORD)wdc_err.WD_STATUS_SUCCESS)//进行判断
            {
