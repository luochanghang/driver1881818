...
namespace Jungo.pcie_diag//定义命名空间
{
    ...
    public class PCIE_diag : System.Windows.Forms.Form
    {
        ...
        public PCIE_diag()    //默认构造方法
        {
            ...
            pciDevList = PCIE_DeviceList.TheDeviceList();     //获取设备列表对象 实例化
        }
...
#region Windows Form Designer generated code
        ...
        private void PCIE_diag_Load(object sender, System.EventArgs e)//当PCIE_diag窗口打开之后进行Load
        {
            DWORD dwStatus = pciDevList.Init();           //将设备列表进行初始化
            if(dwStatus != (DWORD)wdc_err.WD_STATUS_SUCCESS)//检查状态
                goto Error;//如果错误进行错误处理
            
            foreach(PCIE_Device dev in pciDevList)//将pciDevList设备列表对象中的设备加载到界面的选择列表中
                lstBxDevices.Items.Add(dev.ToString(true));
            lstBxDevices.SelectedIndex = 0;//设置默认选择项目
...
Error:
        ...
        {
            pciDevList.Dispose();            //关闭列表
        }        
       /* list box lstBxDevices */
        private void lstBxDevices_SelectedIndexChanged(object sender, //选择框数据更新
            System.EventArgs e)
        {
            if(lstBxDevices.SelectedIndex < 0)//判断当前的选择索引
            {
                DisableMenu();
                btDevice.Enabled = false;
            }
            ...
            {
                PCIE_Device dev = 
                    pciDevList.Get(lstBxDevices.SelectedIndex);//获得当前选择设备
                UpdateMenu(lstBxDevices.SelectedIndex);//更新菜单栏状态
        ...
        private void lstBxDevices_DoubleClicked(object sender, 
            System.EventArgs e)
        {
            btDevice_Click(sender, e); //按钮和双击效果相同
        }
    /* device button */   //打开设备按键处理函数
        ...
        private void DisableMenu()//关闭菜单
        {
            ToggleMenu(false);
        }

        private void ToggleMenu(bool flag)//菜单的具体使能方法
        {
        ...
        {
            PCIE_Device dev =
                pciDevList.Get(lstBxDevices.SelectedIndex);//获得当前选择的设备
    ...
    public class diag_lib
    {
        ...
        public static string DisplayHexBuffer(object[] obj, DWORD dwBuffSize, //打印16进制的缓冲器    obj是一个数组的数组 dwbuffsize是一个数组的大小
             WDC_ADDR_MODE mode)
        {
            string display = "";
            ...
            case WDC_ADDR_MODE.WDC_MODE_8:
                {
                    BYTE[] buff = (BYTE[])obj[0];  //获得数组
                    for(uint i=0; i<dwBuffSize; ++i)//大小比较
                        display = string.Concat(display, 
                            buff[i].ToString("X2"), " ");//display转换成16进制长度两位
                    break;
                }
